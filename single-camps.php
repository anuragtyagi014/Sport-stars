...
$header_image = get_field( "header_image" );
	if ($header_image) {  $headerImageURL = $header_image['url']; }
$logo_image = get_field( "camp_logo" );
$logoImageURL = "";
if ($logo_image) {  $logoImageURL = $logo_image['url']; }
?>
		<!-- jith added -->
    ...
    <div class="body_content clearfix">
      <div class="intro_text">
        <?php $introduction_text = get_field( "introduction_text" );
					if ($introduction_text) { echo $introduction_text; }
				?>
      </div>
      <div class="body_text">
        <?php $introduction_body_text = get_field( "introduction_body_text" );
					if ($introduction_body_text) { echo $introduction_body_text; }
				?>
      </div>
      ...
      <div class="coaches">
        ...
        <div class="coach_name">
          <?php 
                                if ($camp_director_name) { echo $camp_director_name; }
                            ?>
        </div>
    ...
    <?php if ($is_admin || $is_all_star) {   ?>
        <div class="top_buttons clearfix">
          <?php $link_to_video = get_field( "link_to_video" );
                        if ($link_to_video) { echo '<div class="btn_video"><a href="' . $link_to_video .'" target="_blank">Video</a></div>'; }
                    ?>
    ...
    <?php if ($is_admin || $is_all_star) {   ?>
        <div class="social_icons">
          ...
          <?php $camp_fb = get_field( "facebook_link" );
                        if ($camp_fb) { echo '<a href="'.$camp_fb.'" target="_blank"><i class="fa fa-facebook-official fa-3x" aria-hidden="true"></i></a>'; }
                        ?>
          <?php $camp_tw = get_field( "twitter_link" );
                        if ($camp_tw) { echo '<a href="'.$camp_tw.'" target="_blank"><i class="fa fa-twitter fa-3x" aria-hidden="true"></i></a>'; }
                        ?>
          <?php $camp_in = get_field( "instagram_link" );
                        if ($camp_in) { echo '<a href="'.$camp_in.'" target="_blank"><i class="fa fa-instagram fa-3x" aria-hidden="true"></i></a>'; }
                        ?>
          <?php $camp_gp = get_field( "youtube_account_link" );
                        if ($camp_gp) { echo '<a href="'.$camp_gp.'" target="_blank"><i class="fa fa-youtube fa-3x" aria-hidden="true"></i></a>'; }
                        ?>
        </div>
    <?php } ?>
        ...
        <div class="camp_address">
          <?php 
					...
					$camp_postal_code = get_field( "camp_postal_code" );
						...
						if ($camp_phone_number) { echo '<div><a href="tel:'. $camp_phone_number . '">'.$camp_phone_number.'</a></div>'; }
